...
void THPEngine_add_grad(buffer_set_type &need_copy, grad_buffer_type &prev_grad,
    int output_nr, PyObject *d_prev_fn)
{
  ...
  if (!prev_grad[output_nr]) {
    ...
    // TODO: check errors here and above
...
PyObject *THPEngine_run_backward(THPEngine *self, PyObject *args, PyObject *kwargs)
{
  ...
  while (ready.size() > 0) {
    ...
    for (int i = 0; i < num_grads; i++) {
      ...
      // as their _do_backward is supper fast (and we can save memory).
      ...
      if (is_ready) {
        ...
        grad_buffer_type prev_grad(-1);
        if (not_ready_it == not_ready.end()) {
          // The function is ready and no buffers have been allocated for it.
          prev_grad = grad_buffer_type(next_buf_id++, 1);
          Py_INCREF(grad_prev);
          prev_grad[output_idx] = grad_prev;
        } else {
          // The function is ready and it already has a buffer allocated.
          prev_grad = std::move(not_ready_it->second);
          not_ready.erase(not_ready_it);
          THPEngine_add_grad(need_copy, prev_grad, output_idx, grad_prev);
        }
        ...
        THPEngine_add_grad(need_copy, grad_buffer, output_idx, grad_prev);
      }
    }
  }
...
static struct PyMethodDef THPEngine_methods[] = {
  {(char*)"run_backward", (PyCFunction)THPEngine_run_backward, METH_VARARGS, NULL},
  {NULL}
};
